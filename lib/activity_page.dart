...
class _ActivityPageState extends State<ActivityPage> {
  ...
  late List<String> _uniqueThemes = []; // Liste des thèmes uniques
  List<String> _selectedThemes = []; // Liste des thèmes sélectionnés
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.hasError) {
            ...
            _uniqueThemes = activities.map((activity) => activity.theme).toSet().toList();
            return Column(
              ...
              children: [
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: _uniqueThemes.map((theme) {
                      return Padding(
                        ...
                        child: FilterChip(
                          ...
                          backgroundColor: Color.fromARGB(98, 0, 62, 156),
                          selectedColor: Color.fromARGB(255, 0, 62, 156),
                          labelStyle: TextStyle(color: Colors.white),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(20),
                            side: BorderSide(
                              color: _selectedThemes.contains(theme) ? Colors.transparent : Colors.transparent,
                            ),
                          ),
                        ),
                      );
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      // Vérifier si les thèmes sélectionnés correspondent à l'activité
                      if (_selectedThemes.isEmpty || _selectedThemes.contains(activities[index].theme)) {
                        return Padding(
                          padding: const EdgeInsets.all(4.0),
                          child: Material(
                            shape: RoundedRectangleBorder(
                        ...
                        // Retourner un conteneur vide si l'activité ne correspond pas au thème sélectionné
                        return Container();
                      }
                    },
                  ),
